...
int AlgoDetector::extract_yolov5s_tensormem_rockchip(std::vector<rknn_tensor_mem> &output, void *args, std::map<int, std::vector<bbox>> &lastResults)
{
    ...
    for (size_t i = 0; i < output.size(); ++i) {
        validCount += post_process((uint8_t *)(output[i].logical_addr), pArgs, i, filterBoxes, objProbs, classId);
    }
