...
int axpipe_create(axpipe_t *pipe)
{
    ...
    if (media::contain(sg_axPipeHandler.pipeIdPipe, pipe->pipeId)) {
    ...
    switch (pipe->inType) {
        case AXPIPE_INPUT_USER: {
            ...
            if (media::contain(sg_axPipeHandler.ivpsGroup, pipe->ivpsConfig.ivpsGroup)) {
            ...
            if (ret != 0) {
                media::erase(sg_axPipeHandler.ivpsGroup, pipe->ivpsConfig.ivpsGroup);
        ...
        case AXPIPE_INPUT_VIN: {
            if (pipe->vinPipe >= VIN_PIPE_BUTT) {
                axmpi_error("vin pipe must lower than %d, current:[%d]", VIN_PIPE_BUTT - 1, pipe->vinPipe);
                return -7;
            }

            if (pipe->vinChn >= VIN_DEV_CHN_BUTT) {
                axmpi_error("vin chn must lower than %d, current:[%d]", VIN_DEV_CHN_BUTT - 1, pipe->vinChn);
                return -8;
            }
        ...
        case AXPIPE_INPUT_VDEC_JPEG: {
            ...
            if (!media::contain(sg_axPipeHandler.vdecGroup, pipe->vdecConfig.vdecGroup)) {
            ...
            if (ret != 0) {
                media::erase(sg_axPipeHandler.ivpsGroup, pipe->ivpsConfig.ivpsGroup);
                media::erase(sg_axPipeHandler.vdecGroup, pipe->vdecConfig.vdecGroup);
    ...
    switch (pipe->outType) {
        ...
        case AXPIPE_OUTPUT_RTSP_H265: {
            ...
            if (media::contain(sg_axPipeHandler.vencChannel, pipe->vencConfig.vencChannel)) {
            ...
            if (ret != 0) {
                media::erase(sg_axPipeHandler.vencChannel, pipe->vencConfig.vencChannel);
...
int axpipe_release(axpipe_t *pipe)
{
    ...
    media::erase(sg_axPipeHandler.pipeIdPipe, pipe->pipeId);
    ...
    switch (pipe->outType) {
        ...
        case AXPIPE_OUTPUT_RTSP_H265: {
            ...
            if ((pipe->outType == AXPIPE_OUTPUT_RTSP_H264) || (pipe->outType == AXPIPE_OUTPUT_RTSP_H265)) {
                ...
                if (media::contain(sg_axPipeHandler.rtspEndPoint, url)) {
                    media::erase(sg_axPipeHandler.rtspEndPoint, url);
                }
    ...
    switch (pipe->inType) {
        case AXPIPE_INPUT_USER: {
            ...
            for (int i = 0; i < pipes; ++i) {
                if (!media::contain(tmpVec, pipe[i].ivpsConfig.ivpsGroup)) {
        ...
        case AXPIPE_INPUT_VDEC_H264: {
            ...
            for (int i = 0; i < pipes; ++i) {
                if (!media::contain(tmpVec, pipe[i].vdecConfig.vdecGroup)) {
