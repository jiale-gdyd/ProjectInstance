#ifndef ACL_LIBACL_STDLIB_ACL_VSTREAM_H
#define ACL_LIBACL_STDLIB_ACL_VSTREAM_H
 ...
 */
ACL_API ACL_VSTREAM *acl_vstream_fdopen(ACL_SOCKET fd, unsigned int oflags, size_t buflen, int rw_timeout, int fdtype);
 ...
 *  ret > 0:  表示成功写了 dlen 个字节的数据
 */
ACL_API int ACL_PRINTF(2, 3) acl_vstream_buffed_fprintf(ACL_VSTREAM *fp, const char *fmt, ...);
 ...
 *  ret > 0:  表示成功写了 dlen 个字节的数据
 */
ACL_API int ACL_PRINTF(2, 3) acl_vstream_fprintf(ACL_VSTREAM *fp, const char *fmt, ...);
