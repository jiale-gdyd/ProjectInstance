#ifndef ACL_LIBACL_STDLIB_ACL_RING_H
#define ACL_LIBACL_STDLIB_ACL_RING_H
...
#define	ACL_RING_FOREACH(iter, head_ptr) \
    for ((iter).ptr = acl_ring_succ((head_ptr)); (iter).ptr != (head_ptr); (iter).ptr = acl_ring_succ((iter).ptr))
...
#define	ACL_RING_FOREACH_REVERSE(iter, head_ptr) \
    for ((iter).ptr = acl_ring_pred((head_ptr)); (iter).ptr != (head_ptr); (iter).ptr = acl_ring_pred((iter).ptr))
...
#define ACL_RING_FIRST_APPL(head, app_type, ring_member) \
    (acl_ring_succ(head) != (head) ? ACL_RING_TO_APPL(acl_ring_succ(head), app_type, ring_member) : 0)
...
#define ACL_RING_LAST_APPL(head, app_type, ring_member) \
    (acl_ring_pred(head) != (head) ? ACL_RING_TO_APPL(acl_ring_pred(head), app_type, ring_member) : 0)
