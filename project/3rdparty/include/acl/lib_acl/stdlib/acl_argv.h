#ifndef ACL_LIBACL_STDLIB_ACL_ARGV_H
#define ACL_LIBACL_STDLIB_ACL_ARGV_H
 ...
 * @param str   {const char*} 源字符串
 * @param delim {const char*} 分隔字符串
 * @param dbuf  {ACL_DBUF_POOL*} 内存池对象，可以为空，当为空时则采用缺省的内存分配方式
 * @return      {ACL_ARGV*}
 */
...
/**
 * 根据源字符串及分隔字符串生成一个字符串动态数组，针对由 "" 或 '' 引用的字符串不做分隔
 * @param str   {const char*} 源字符串
 * @param delim {const char*} 分隔字符串
 * @return      {ACL_ARGV*}
 */
