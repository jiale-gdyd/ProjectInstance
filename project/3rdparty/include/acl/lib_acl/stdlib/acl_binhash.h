#ifndef ACL_LIBACL_STDLIB_ACL_BINHASH_H
#define ACL_LIBACL_STDLIB_ACL_BINHASH_H
 ...
 * 列出当前哈希表中的所有元素数组列表
 * @param table {ACL_BINHASH*} 哈希表指针
 * @return      {ACL_BINHASH_INFO*} 哈希表中所有元素组成的ACL_BINHASH_INFO数组, 该数组中的最后一个指针为 NULL
 */
...
#define ACL_BINHASH_FOREACH(iter, table_ptr)  \
    if (table_ptr)  \
        for((void) acl_binhash_iter_head((table_ptr), &iter); (iter).ptr; (void) acl_binhash_iter_next(&iter))
#define ACL_BINHASH_FOREACH_REVERSE(iter, table_ptr)  \
    if (table_ptr)  \
        for((void) acl_binhash_iter_tail((table_ptr), &iter); (iter).ptr; (void) acl_binhash_iter_prev(&iter))
#endif
