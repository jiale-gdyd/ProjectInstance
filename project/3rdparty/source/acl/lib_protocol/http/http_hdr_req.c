...
#include "acl/lib_acl/stdlib/acl_iterator.h"
#include "http.h"
...
#define    TRUNC_BLANK_NORETURN(_x_) do {  \
    ...
    /* ��Щվ���COOKIE�Ƚ��������Ѷ��reg.hexun.com��COOKIE�������ظ�
     * ����������Ա����ж�һ�£������ظ��洢��ͬ���ֵ�COOKIEֵ�������
     * �����ظ�COOKIE������ֻ�洢��һ���������ͱ����˲��ù�ϣ��ʽ�洢��
     * ©�ڴ����������--- zsx, 2008.1.8
     */
...
int http_hdr_req_cookies_parse(HTTP_HDR_REQ *hh)
{
    ...
    /* bugfix: �ڴ�����ϣ��ʱ�˴���Ӧ���� ACL_HTABLE_FLAG_KEY_REUSE ��־λ��
     * ��������˴˱�־������ __add_cookie_item �е��� acl_htable_enter ʱ��
     * �Ὣ name ֵ���ڴ潻�� htable��������ں� RETURN ʱȴ�������ͷ�����
     * �ĺ��� acl_argv_free(argv)���� name �����������ڴ�һ����ͷ���
     * ---zsx, 2014.5.13
     */
...
static void __strip_url_path(ACL_VSTRING *buf, const char *url)
{
    ...
    /* xxx: ���뽫�������еĳ�ʼ������ acl_argv_split �ĺ��棬��Ϊ url
     * ��ָ�����п����� buf �еĻ�������ַ��ͬ���μ� __strip_url_path
     * ���������ã����⣬�ڵ��� ACL_VSTRING_RESET �󻹱������
     * ACL_VSTRING_TERMINATE������ ACL_VSTRING_RESET ���ƶ�ָ��λ�ã�
     * �����Ὣ��ʼλ�ø� '\0'
     */
...
int http_hdr_req_rewrite2(HTTP_HDR_REQ *hh, const char *url)
{
    ...
    if (strncasecmp(url, "http://", strlen("http://")) == 0) {
        ...
        phost = hh->host;  /* ���URL��û�� http[s]:// ��Ĭ�ϲ���ԭ Host �ֶ� */
        purl = url;
    }
    host[0] = 0;
...
/* ��ȡ����URL��ĳ�������ֶε�����, ��ȡ: /cgi-bin/n1=v1&n2=v2 �е� n2��ֵv2 */
    ...
    http_off_t *range_to)
{
    ...
    /* ���ݸ�ʽ: Range: bytes={range_from}-{range_to}
