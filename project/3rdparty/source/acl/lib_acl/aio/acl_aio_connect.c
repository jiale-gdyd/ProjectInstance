...
#include "../event/events_define.h"
#include "aio.h"

#define	WRITE_SAFE_ENABLE(x, callback) \
    do {  \
...
static void __connect_notify_callback(int event_type, ACL_EVENT *event, ACL_VSTREAM *stream acl_unused, void *context)
{
...
int acl_aio_connect_addr(ACL_AIO *aio, const char *addr, int timeout, ACL_AIO_CONNECT_ADDR_FN callback, void *context)
{
    char buf[128], *ptr;
    int  port;
    RESOLVE_CTX *ctx;
