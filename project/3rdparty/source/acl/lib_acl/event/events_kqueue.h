#ifndef ACL_LIBACL_EVENT_EVENTS_KQUEUE_H
#define ACL_LIBACL_EVENT_EVENTS_KQUEUE_H
...
*/
#ifndef EV_SET
#define EV_SET(kp, id, fi, fl, ffl, da, ud) \
    do { \
        (kp)->ident = (id); \
        (kp)->filter = (fi); \
        (kp)->flags = (fl); \
...
#define EVENT_REG_INIT_HANDLE(eh, n) \
    do { \
        eh = kqueue(); \
    } while (0)
...
*/
#define EVENT_REG_FD_OP(er, eh, fh, ctx, ev, op) \
    do { \
        struct kevent dummy; \
...
#define EVENT_BUFFER_READ(ev_cnt, eh, ev_buf, buflen, delay) \
    do { \
        struct timespec ts; \
        struct timespec *tsp; \
        if ((delay) < 0) { \
