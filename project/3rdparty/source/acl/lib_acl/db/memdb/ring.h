#ifndef ACL_LIB_ACL_DB_MEMDB_RING_H
#define ACL_LIB_ACL_DB_MEMDB_RING_H
...
#define	ring_init(ring_in) \
    do {  \
        RING   *ring = ring_in;  \
        ring->pred = ring->succ = ring;  \
    } while (0)

#define	ring_append(ring_in, entry_in) \
    do {  \
        RING   *ring = ring_in;  \
        RING   *entry = entry_in;  \
...
#define	ring_prepend(ring_in, entry_in) \
    do {  \
        RING   *ring = ring_in;  \
        RING   *entry = entry_in;  \
...
#define FOREACH_TO_APPL_ENTRY(entry, head) \
    for (entry = ring_succ(head); entry != (head); entry = ring_succ(entry))

#define FOREACH_RING_ENTRY(entry, head) \
    for (entry = ring_succ(head); entry != (head); entry = ring_succ(entry))

#define FOREACH_RING_FORWARD    FOREACH_RING_ENTRY
...
#define FIRST_APPL(head, app_type, ring_member) \
    (ring_succ(head) != (head) ? RING_TO_APPL(ring_succ(head), app_type, ring_member) : 0)
