...
#define SHA2_BLOCK_LEN         128
#define SHA384_DIGEST_LEN       48
#define SHA512_DIGEST_LEN       64

#define IS_VALID_TYPE(type)     ((type) >= X_CHECKSUM_MD5 && (type) <= X_CHECKSUM_SHA384)

static const xchar hex_digits[] = "0123456789abcdef";

typedef struct {
    xuint32     buf[4];
    xuint32     bits[2];
    union {
        xuchar  data[MD5_DATASIZE];
        xuint32 data32[MD5_DATASIZE / 4];
    } u;
    xuchar      digest[MD5_DIGEST_LEN];
} Md5sum;
...
typedef struct {
    ...
    xuint32 bits[2];
    xuint32 data[16];
    xuchar  digest[SHA1_DIGEST_LEN];
} Sha1sum;

typedef struct {
    xuint32 buf[8];
    xuint32 bits[2];
    xuint8  data[SHA256_DATASIZE];
    xuchar  digest[SHA256_DIGEST_LEN];
} Sha256sum;

typedef struct {
    xuint64 H[8];
    xuint8  block[SHA2_BLOCK_LEN];
    xuint8  block_len;
    xuint64 data_len[2];
    xuchar  digest[SHA512_DIGEST_LEN];
} Sha512sum;
...
#else
static inline void md5_byte_reverse(xuchar *buffer, xulong length)
{
    xuint32 bit;

    do {
        bit = (xuint32) ((unsigned)buffer[3] << 8 | buffer[2]) << 16 | ((unsigned)buffer[1] << 8 | buffer[0]);
        * (xuint32 *)buffer = bit;
        buffer += 4;
    } while (--length);
}
...
#else
static inline void sha_byte_reverse(xuint32 *buffer, xint length)
{
    length /= sizeof(xuint32);
    while (length--) {
        *buffer = XUINT32_SWAP_LE_BE(*buffer);
        ++buffer;
    }
}
#endif

static xchar *digest_to_string(xuint8 *digest, xsize digest_len)
{
    xchar *retval;
    xsize i, len = digest_len * 2;

    retval = x_new(xchar, len + 1);

    for (i = 0; i < digest_len; i++) {
        xuint8 byte = digest[i];
...
static void md5_transform(xuint32 buf[4], xuint32 const in[16])
{
    xuint32 a, b, c, d;
...
static void md5_sum_update(Md5sum *md5, const xuchar *data, xsize length)
{
    xuint32 bit;

    bit = md5->bits[0];
    md5->bits[0] = bit + ((xuint32)length << 3);
...
static void md5_sum_close(Md5sum *md5)
{
    ...
    *p++ = 0x80;
    ...
    md5_transform (md5->buf, md5->u.data32);
    md5_byte_reverse((xuchar *) md5->buf, 4);
...
static xchar *md5_sum_to_string(Md5sum *md5)
{
    return digest_to_string(md5->digest, MD5_DIGEST_LEN);
}

static void md5_sum_digest(Md5sum *md5, xuint8 *digest)
{
    xint i;
...
static void sha1_transform(xuint32 buf[5], xuint32 in[16])
{
    xuint32 A, B, C, D, E;
...
static void sha1_sum_update(Sha1sum *sha1, const xuchar *buffer, xsize count)
{
    xuint32 tmp;
    xuint dataCount;

    tmp = sha1->bits[0];
    if ((sha1->bits[0] = tmp + ((xuint32)count << 3)) < tmp) {
        sha1->bits[1] += 1;
    }
    sha1->bits[1] += count >> 29;

    dataCount = (xuint) (tmp >> 3) & 0x3F;
    if (dataCount) {
        xuchar *p = (xuchar *) sha1->data + dataCount;
...
static void sha1_sum_close(Sha1sum *sha1)
{
    ...
    count = (xint)((sha1->bits[0] >> 3) & 0x3f);

    data_p = (xuchar *)sha1->data + count;
    *data_p++ = 0x80;
...
static xchar *sha1_sum_to_string(Sha1sum *sha1)
{
    return digest_to_string(sha1->digest, SHA1_DIGEST_LEN);
}

static void sha1_sum_digest(Sha1sum *sha1, xuint8 *digest)
{
    xint i;
...
static void sha256_transform(xuint32 buf[8], xuint8 const data[64])
{
    xuint32 temp1, temp2, W[64];
    xuint32 A, B, C, D, E, F, G, H;
...
static void sha256_sum_update(Sha256sum *sha256, const xuchar *buffer, xsize length)
{
    xuint32 left, fill;
    const xuint8 *input = buffer;
...
static xuint8 sha256_padding[64] = {
...
static xchar *sha256_sum_to_string(Sha256sum *sha256)
{
    return digest_to_string(sha256->digest, SHA256_DIGEST_LEN);
}

static void sha256_sum_digest(Sha256sum *sha256, xuint8 *digest)
{
    xint i;
...
static const xuint64 SHA2_K[80] = {
    X_XUINT64_CONSTANT(0x428a2f98d728ae22), X_XUINT64_CONSTANT(0x7137449123ef65cd),
    X_XUINT64_CONSTANT(0xb5c0fbcfec4d3b2f), X_XUINT64_CONSTANT(0xe9b5dba58189dbbc),
    X_XUINT64_CONSTANT(0x3956c25bf348b538), X_XUINT64_CONSTANT(0x59f111f1b605d019),
    X_XUINT64_CONSTANT(0x923f82a4af194f9b), X_XUINT64_CONSTANT(0xab1c5ed5da6d8118),
    X_XUINT64_CONSTANT(0xd807aa98a3030242), X_XUINT64_CONSTANT(0x12835b0145706fbe),
    X_XUINT64_CONSTANT(0x243185be4ee4b28c), X_XUINT64_CONSTANT(0x550c7dc3d5ffb4e2),
    X_XUINT64_CONSTANT(0x72be5d74f27b896f), X_XUINT64_CONSTANT(0x80deb1fe3b1696b1),
    X_XUINT64_CONSTANT(0x9bdc06a725c71235), X_XUINT64_CONSTANT(0xc19bf174cf692694),
    X_XUINT64_CONSTANT(0xe49b69c19ef14ad2), X_XUINT64_CONSTANT(0xefbe4786384f25e3),
    X_XUINT64_CONSTANT(0x0fc19dc68b8cd5b5), X_XUINT64_CONSTANT(0x240ca1cc77ac9c65),
    X_XUINT64_CONSTANT(0x2de92c6f592b0275), X_XUINT64_CONSTANT(0x4a7484aa6ea6e483),
    X_XUINT64_CONSTANT(0x5cb0a9dcbd41fbd4), X_XUINT64_CONSTANT(0x76f988da831153b5),
    X_XUINT64_CONSTANT(0x983e5152ee66dfab), X_XUINT64_CONSTANT(0xa831c66d2db43210),
    X_XUINT64_CONSTANT(0xb00327c898fb213f), X_XUINT64_CONSTANT(0xbf597fc7beef0ee4),
    X_XUINT64_CONSTANT(0xc6e00bf33da88fc2), X_XUINT64_CONSTANT(0xd5a79147930aa725),
    X_XUINT64_CONSTANT(0x06ca6351e003826f), X_XUINT64_CONSTANT(0x142929670a0e6e70),
    X_XUINT64_CONSTANT(0x27b70a8546d22ffc), X_XUINT64_CONSTANT(0x2e1b21385c26c926),
    X_XUINT64_CONSTANT(0x4d2c6dfc5ac42aed), X_XUINT64_CONSTANT(0x53380d139d95b3df),
    X_XUINT64_CONSTANT(0x650a73548baf63de), X_XUINT64_CONSTANT(0x766a0abb3c77b2a8),
    X_XUINT64_CONSTANT(0x81c2c92e47edaee6), X_XUINT64_CONSTANT(0x92722c851482353b),
    X_XUINT64_CONSTANT(0xa2bfe8a14cf10364), X_XUINT64_CONSTANT(0xa81a664bbc423001),
    X_XUINT64_CONSTANT(0xc24b8b70d0f89791), X_XUINT64_CONSTANT(0xc76c51a30654be30),
    X_XUINT64_CONSTANT(0xd192e819d6ef5218), X_XUINT64_CONSTANT(0xd69906245565a910),
    X_XUINT64_CONSTANT(0xf40e35855771202a), X_XUINT64_CONSTANT(0x106aa07032bbd1b8),
    X_XUINT64_CONSTANT(0x19a4c116b8d2d0c8), X_XUINT64_CONSTANT(0x1e376c085141ab53),
    X_XUINT64_CONSTANT(0x2748774cdf8eeb99), X_XUINT64_CONSTANT(0x34b0bcb5e19b48a8),
    X_XUINT64_CONSTANT(0x391c0cb3c5c95a63), X_XUINT64_CONSTANT(0x4ed8aa4ae3418acb),
    X_XUINT64_CONSTANT(0x5b9cca4f7763e373), X_XUINT64_CONSTANT(0x682e6ff3d6b2b8a3),
    X_XUINT64_CONSTANT(0x748f82ee5defb2fc), X_XUINT64_CONSTANT(0x78a5636f43172f60),
    X_XUINT64_CONSTANT(0x84c87814a1f0ab72), X_XUINT64_CONSTANT(0x8cc702081a6439ec),
    X_XUINT64_CONSTANT(0x90befffa23631e28), X_XUINT64_CONSTANT(0xa4506cebde82bde9),
    X_XUINT64_CONSTANT(0xbef9a3f7b2c67915), X_XUINT64_CONSTANT(0xc67178f2e372532b),
    X_XUINT64_CONSTANT(0xca273eceea26619c), X_XUINT64_CONSTANT(0xd186b8c721c0c207),
    X_XUINT64_CONSTANT(0xeada7dd6cde0eb1e), X_XUINT64_CONSTANT(0xf57d4f7fee6ed178),
    X_XUINT64_CONSTANT(0x06f067aa72176fba), X_XUINT64_CONSTANT(0x0a637dc5a2c898a6),
    X_XUINT64_CONSTANT(0x113f9804bef90dae), X_XUINT64_CONSTANT(0x1b710b35131c471b),
    X_XUINT64_CONSTANT(0x28db77f523047d84), X_XUINT64_CONSTANT(0x32caab7b40c72493),
    X_XUINT64_CONSTANT(0x3c9ebe0a15c9bebc), X_XUINT64_CONSTANT(0x431d67c49c100d4c),
    X_XUINT64_CONSTANT(0x4cc5d4becb3e42b6), X_XUINT64_CONSTANT(0x597f299cfc657e2a),
    X_XUINT64_CONSTANT(0x5fcb6fab3ad6faec), X_XUINT64_CONSTANT(0x6c44198c4a475817)
};

static void sha384_sum_init(Sha512sum *sha512)
{
    sha512->H[0] = X_XUINT64_CONSTANT(0xcbbb9d5dc1059ed8);
    sha512->H[1] = X_XUINT64_CONSTANT(0x629a292a367cd507);
    sha512->H[2] = X_XUINT64_CONSTANT(0x9159015a3070dd17);
    sha512->H[3] = X_XUINT64_CONSTANT(0x152fecd8f70e5939);
    sha512->H[4] = X_XUINT64_CONSTANT(0x67332667ffc00b31);
    sha512->H[5] = X_XUINT64_CONSTANT(0x8eb44a8768581511);
    sha512->H[6] = X_XUINT64_CONSTANT(0xdb0c2e0d64f98fa7);
    sha512->H[7] = X_XUINT64_CONSTANT(0x47b5481dbefa4fa4);
...
{
    sha512->H[0] = X_XUINT64_CONSTANT(0x6a09e667f3bcc908);
    sha512->H[1] = X_XUINT64_CONSTANT(0xbb67ae8584caa73b);
    sha512->H[2] = X_XUINT64_CONSTANT(0x3c6ef372fe94f82b);
    sha512->H[3] = X_XUINT64_CONSTANT(0xa54ff53a5f1d36f1);
    sha512->H[4] = X_XUINT64_CONSTANT(0x510e527fade682d1);
    sha512->H[5] = X_XUINT64_CONSTANT(0x9b05688c2b3e6c1f);
    sha512->H[6] = X_XUINT64_CONSTANT(0x1f83d9abfb41bd6b);
    sha512->H[7] = X_XUINT64_CONSTANT(0x5be0cd19137e2179);
...
static void sha512_transform(xuint64 H[8], xuint8 const data[SHA2_BLOCK_LEN])
{
    xint i;
    xint t;
    xuint64 M[16];
    xuint64 W[80];
    xuint64 a, b, c, d, e, f, g, h;
...
static void sha512_sum_update(Sha512sum *sha512, const xuchar *buffer, xsize length)
{
    xsize block_left, offset = 0;
...
static xchar *sha384_sum_to_string(Sha512sum *sha512)
{
    return digest_to_string(sha512->digest, SHA384_DIGEST_LEN);
}

static xchar *sha512_sum_to_string(Sha512sum *sha512)
{
    return digest_to_string(sha512->digest, SHA512_DIGEST_LEN);
}

static void sha384_sum_digest(Sha512sum *sha512, xuint8 *digest)
{
    memcpy(digest, sha512->digest, SHA384_DIGEST_LEN);
}

static void sha512_sum_digest (Sha512sum *sha512, xuint8 *digest)
{
...
XChecksum *x_checksum_new(XChecksumType checksum_type)
{
    ...
    checksum = x_slice_new0(XChecksum);
    checksum->type = checksum_type;

    x_checksum_reset(checksum);
    return checksum;
}

void x_checksum_reset(XChecksum *checksum)
{
    x_return_if_fail(checksum != NULL);

    x_free(checksum->digest_str);
...
XChecksum *x_checksum_copy(const XChecksum *checksum)
{
    ...
    copy = x_slice_new(XChecksum);
    *copy = *checksum;
    copy->digest_str = x_strdup(checksum->digest_str);
...
{
    if (X_LIKELY(checksum)) {
        x_free(checksum->digest_str);
        x_slice_free(XChecksum, checksum);
    }
}
...
void x_checksum_update(XChecksum *checksum, const xuchar *data, xssize length)
{
    ...
    if (length < 0) {
        length = strlen((const xchar *)data);
    }

    if (checksum->digest_str) {
        x_warning("The checksum '%s' has been closed and cannot be updated " "anymore.", checksum->digest_str);
        return;
    }
...
void x_checksum_get_digest(XChecksum *checksum, xuint8 *buffer, xsize *digest_len)
{
    xsize len;
    xchar *str = NULL;
    xboolean checksum_open = FALSE;

    x_return_if_fail(checksum != NULL);
    len = x_checksum_type_get_length(checksum->type);
...
xchar *x_compute_checksum_for_data(XChecksumType checksum_type, const xuchar *data, xsize length)
{
    ...
    checksum = x_checksum_new(checksum_type);
    if (!checksum) {
        return NULL;
    }

    x_checksum_update(checksum, data, length);
    retval = x_strdup(x_checksum_get_string(checksum));
    x_checksum_free(checksum);
...
xchar *x_compute_checksum_for_string(XChecksumType checksum_type, const xchar *str, xssize length)
{
    ...
    return x_compute_checksum_for_data(checksum_type, (const xuchar *) str, length);
}
...
xchar *x_compute_checksum_for_bytes(XChecksumType checksum_type, XBytes *data)
{
    ...
    return x_compute_checksum_for_data(checksum_type, (const xuchar *)byte_data, length);
}
