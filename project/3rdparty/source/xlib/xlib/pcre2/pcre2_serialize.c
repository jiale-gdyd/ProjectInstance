...
bytes = (uint8_t *)memctl->malloc(total_size + sizeof(pcre2_memctl), memctl->memory_data);
...
tables = (uint8_t *)memctl->malloc(TABLES_LENGTH + sizeof(PCRE2_SIZE), memctl->memory_data);
