...
overflow. */
  ...
  newt =(RWS_anchor *)mb->memctl.malloc(newsize*sizeof(int), mb->memctl.memory_data);
  if (newt == NULL) return PCRE2_ERROR_NOMEMORY;
...
newt->free = newt->size - RWS_ANCHOR_SIZE;
*rwsptr = newt;
return 0;
}
...
for (;;)
      ...
      if (has_first_cu)
        ...
        if (first_cu != first_cu2)  /* Caseless */
          ...
          if (memchr_found_first_cu == NULL ||
            ...
            {
            pp1 = (PCRE2_SPTR)memchr(start_match, first_cu, searchlength);
          ...
          if (memchr_found_first_cu2 == NULL ||
            ...
            {
            pp2 = (PCRE2_SPTR)memchr(start_match, first_cu2, searchlength);
        ...
        /* The caseful case is much simpler. */
          ...
          start_match = (PCRE2_SPTR)memchr(start_match, first_cu, end_subject - start_match);
...
#else  /* 8-bit code units */
            PCRE2_SPTR pp = p;
            p = (PCRE2_SPTR)memchr(pp, req_cu, end_subject - pp);
            if (p == NULL)
              {
              p = (PCRE2_SPTR)memchr(pp, req_cu2, end_subject - pp);
...
#else  /* 8-bit code units */
            p = (PCRE2_SPTR)memchr(p, req_cu, end_subject - p);
            if (p == NULL) p = end_subject;
#endif
            }
  ...
  if (rc != PCRE2_ERROR_NOMATCH || anchored)
    ...
    if (rc >= 0 &&(options & PCRE2_COPY_MATCHED_SUBJECT) != 0)
      ...
      match_data->subject = (PCRE2_SPTR)match_data->memctl.malloc(length,
        match_data->memctl.memory_data);
