...
RtspConnection::RtspConnection(std::shared_ptr<Rtsp> rtsp, TaskScheduler *task_scheduler, SOCKET sockfd)
    : TcpConnection(task_scheduler, sockfd), rtsp_(rtsp), task_scheduler_(task_scheduler), rtp_channel_(new Channel(sockfd)), rtsp_request_(new RtspRequest), rtsp_response_(new RtspResponse)
{
    ...
    rtp_channel_->SetWriteCallback([this]() {
        this->HandleWrite();
    });

    rtp_channel_->SetCloseCallback([this]() {
        this->HandleClose();
    });

    rtp_channel_->SetErrorCallback([this]() {
        this->HandleError();
    });
    ...
    if (media_session->IsMulticast()) {
        ...
        if (rtsp_request_->GetTransportMode() == RTP_OVER_TCP) {
            ...
            if (rtp_conn_->SetupRtpOverUdp(channel_id, peer_rtp_port, peer_rtcp_port)) {
                ...
                rtcp_channels_[channel_id]->SetReadCallback([rtcp_fd, this]() {
