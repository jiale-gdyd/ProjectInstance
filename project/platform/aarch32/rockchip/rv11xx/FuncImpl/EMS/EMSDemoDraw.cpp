...
{
    getApi()->getRgn().putText(0, "有目标啦 - ^_^", cv::Point(20, 50), 50, CV_RGB(255, 0, 255));
    for (auto item : lastResult) {
        getApi()->getRgn().drawRect(cv::Point(item.left, item.top), cv::Point(item.right, item.bottom), CV_RGB(255, 0, 0), 3);
    }
    ...
    if (!mBlendImage.empty()) {
        ...
        getApi()->getRgn().drawImage(mBlendImage, cv::Point(startX, startY), iconW, iconH);
    }
